...
export const runShellTool = defineTool('run_shell', 'Execute a shell command. Returns stdout and stderr. Use with caution - only safe commands are allowed by default.', {
    type: 'object',
    properties: {
        command: {
            type: 'string',
    ...
    if (!isCommandAllowed(command, allowedCommands)) {
        throw new PermissionError(`Command "${command}" is not in the allowed list. Allowed commands: ${allowedCommands.join(', ')}`);
    }
    if (context.dryRun) {
